...
import math as m
l1 = tuple(input("Enter first point with latitude and longitude :").split(","))
l2 = tuple(input("Enter second point with latitude and longitude :").split(","))
t1 = m.radians(float(l1[0]))
g1 = m.radians(float(l1[1]))
t2 = m.radians(float(l2[0]))
g2 = m.radians(float(l2[0]))

distance = 6371.01 * m.acos(m.sin(t1) * m.sin(t2) + m.cos(t1) * m.cos(t2) * m.cos(g1-g2))

print("Distance in Kilometer is  : %.2f" %distance)
